...
MODULE_DESCRIPTION("Apogee CCD camera parallel port Interface");
MODULE_LICENSE("GPL");
#endif
...
int 
apppi_open(struct inode *, struct file *);
int apppi_ioctl(struct inode *, struct file *, unsigned int, unsigned long);
static ssize_t apppi_write(struct file * file, const char * buffer, 
			   size_t count, loff_t *ppos);
static ssize_t apppi_read(struct file * file, char * buffer, 
			  size_t count, loff_t *ppos);
static int apppi_release(struct inode *, struct file *);


struct file_operations apppi_fops = {
     ...
     if (apppi_addr[i]) {
        ...
        for(j=0;j<MAXBOARDS;j++) {
          if (apppi_addr[i] == bases[j]) {
             if(check_region(apppi_addr[i],32)<0)
             {
               printk("<1>apppi I/O region (0x%X) is already reserved by another device\n",apppi_addr[i]);
               continue;
             }
