...
int check_input_file(void)
{
	...
	while(!feof(fin))
	{
		...
		if(open_flag[0]=='y' || open_flag[0]=='Y')
		{
			...
			/*
			if(abs(start.timestamp - prev_start_timestamp)<FIVEMINUTES)
			{printf("%s Two Start times are too close (difference must be > 5 min) (Check %s)\n",error_string,start_string);Nerrors++;}
			*/
				...
				int ccd_temp, char *filter_string, char flat_flag)
{
	...
	if(end.timestamp - start.timestamp < FIVEMINUTES)
		{printf("%s End - Start time < 5 minutes ! (%s - %s)\n",error_string,start.string,end.string);Nerrors++;}
	*/
