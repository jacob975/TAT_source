...
int main(int argc,char* argv[])
{
	//--------------------------------------------------------------
	// Declaration
	// For communication with hardwares 
	...
	int	delay;
	char open_flag[2], open_string[13], close_string[13];
	...
	// For planning observation procedure
	...
	//---------------------------------------------------------------------------------
	// Using the number of argument to determine where the telescope start.
	// 1 argument: The telescope will move from home position to the next target.
	// 3 arguments: The telescope will move from current position to the next target.	
	if (argc == 3)
	{
	...
	//------------------------------------------------------------	
	// Begin to find a observable target
	...
	if(Current_pos.ra<0) //Current position is home switch
	{
		...
		sprintf(
			temp_string,
			"Reference position refer to zenith: RA(deg)=%10.5f,  DEC(deg)=%10.5f", 
			Ra.deg_begin,
			Dec.deg_begin
		);
		steplog(temp_string, STAR_TRACK_LOG_TYPE);
		sprintf(
			temp_string,
			"Reference position: Hour Angle(hours)= %7.5f, Dec(deg)=%7.5f",
			(float)-1*Ra.deg_begin/15.0,
			LOCAL_LAT - Dec.deg_begin	
		);
		steplog(temp_string, STAR_TRACK_LOG_TYPE);
	}
	// Current position is unknown but given by the arguements.
	else{ 
		sprintf(
			temp_string,
			"Current position: RA(hour)=%10.5f,  DEC(deg)=%10.5f", 
			Current_pos.ra,
			Current_pos.dec
		);
		steplog(temp_string,STAR_TRACK_LOG_TYPE);
	...
	while(!feof(fin1))
	{
		...
		i = sscanf(
			...
			target_name);
		//---------------------------------------------------------
		// Check whether this target is observable now.
		...
		sprintf(
			temp_string,
			"Open Time (UT) = %4d/%02d/%02d %02d:%02d\n", 
			open.yr,
			open.mon,
			open.day,
			open.hr,
		...
		sprintf(
			temp_string,
			"%sClose Time (UT) = %4d/%02d/%02d %02d:%02d", 
			temp_string,
			close.yr, 
			close.mon, 
			close.day, 
		...
		sprintf(
			temp_string,
			"Target's name: %s\nTarget coordinates:  RA = %2dh %2dm %2ds  DEC = %3dd %3dm %3ds", 
			target_name,
			ra_h, 
			ra_m, 
			ra_s, 
		...
		steplog(temp_string,STAR_TRACK_LOG_TYPE);
		//------------------------------------------
		// Load the filter sequence and exposure time
		N_filters = read_filter_string(
			filter_string,
			filter_seq,
			filter_exp_time,
			filter_obs_time
		...
		steplog(temp_string,STAR_TRACK_LOG_TYPE);
		//----------------------------------------------
		// Estimate the total observing time
		total_obs_time = close.timestamp - now.timestamp;
		...
		steplog(temp_string,STAR_TRACK_LOG_TYPE);
		// Upload the total observing time and the number of cycle to shared memory.
	...
	if(!(IsObservable)){
		sprintf(
			temp_string,
			"ERROR: It is not observing time!\nCheck \"%s\"",
			TIME_TABLE_FILENAME
		);
		steplog(temp_string,STAR_TRACK_LOG_TYPE);
		return 1;
	...
	*/
	...
	//-------------------------------------------------------------------------
	// Calculating the offset between current position and the next target.
	mv(lt, StarPos,Current_pos, &Ra.deg0_mv,&Dec.deg0_mv);
	...
	//sprintf(
	//	temp_string,
	//	"Star_track: RA offset %8.2f DEC offset %8.2f Obs = %s",
	//	Ra.deg0_mv, 
	//	Dec.deg0_mv, 
	//	filter_string
	//);
	...
	sprintf(
		temp_string,
		"Motor Speed 1 (Hz): RA= %8.2f; DEC= %8.2f\n", 
		Ra.motor_speed1, 
		Dec.motor_speed1
	);
	sprintf(
		temp_string,
		"%sMotor Speed 2 (Hz): RA= %8.2f; DEC= %8.2f\n",
		temp_string, 
		Ra.motor_speed2, 
		Dec.motor_speed2
	);
	...
	sprintf(
		temp_string, 
		"Distance from Current position to next target(deg): RA= %7.2f; DEC= %7.2f", 
...
/********************************************************************/
/* subroutine:
	void mv(DATE d, STARLIST star_pos, ra_deg_mv, dec_deg_mv)
...
int mv(DATE d, STARLIST star_pos, STARLIST Current_pos, float *ra_deg_mv, float *dec_deg_mv)
{
	...
	//--------------------------------------------------------
	// Compute the hour angle (HA) of the object respected to now:
	hourAngle_obj	= LST - star_pos.ra;
	dec_deg_obj	= star_pos.dec;
	sprintf(temp_string, "Hour Angle = %7.5f, DEC = %7.5f", hourAngle_obj, dec_deg_obj);
	step(temp_string);
